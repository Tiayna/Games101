...
{   
    ...
    //do Cross Product to judge whether inside
    //Formula:(x1,y1)x(x2,y2) = x1*y2-x2*y1
    //if all the cross product are positive or negative,that inside

    //A = _v[0]  B = _v[1]  C = _v[2]  P = (x,y)
    //Triangle's sides
    ...
    //Judge
    if(z1>0&&z2>0&&z3>0||z1<0&&z2<0&&z3<0) return true;     //inside
    else return false;   //not inside
}

static std::tuple<float, float, float> computeBarycentric2D(float x, float y, const Vector3f* v)    //return lvalue_reference tuple
{
...
//Suppose the mvp and viewport have finished
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    bool MSAA = true;   //whether use MSAA
    if (MSAA) 
    {
    ...
    else
    {
        ...
        for(int x = x_min;x <= x_max;x++)
        {
            ...
            {
                if(insideTriangle(x,y,t.v))    //Z-Buffer
                {
                    float min_depth = FLT_MAX;   //Infinity

                    float alpha,beta,gamma;  //Interpolation
